...
export function AgencyResetPasswordPage() {
  ...
  async function onForgotSubmit(data: ForgotData) {
    setServerError(null)
    try {
      await api.post('/api/v1/agency/auth/forgot-password', data)
      setStatus('success')
    } catch (err: unknown) {
  ...
  async function onResetSubmit(data: ResetData) {
    setServerError(null)
    try {
      await api.post('/api/v1/agency/auth/reset-password', {
        token,
        newPassword: data.password,
      })
      await navigate({ to: '/agency/login' })
    } catch (err: unknown) {
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 text-center">
              <Link to="/agency/login" className="text-xs text-blue-steel hover:underline">
                Voltar ao login
              </Link>
            </div>
          </CardContent>
